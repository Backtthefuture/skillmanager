 ...
 */
export function tokenize(text, synonymIndex) {
    const out = new Set();
    if (!text)
        return out;
 ...
 */
function signatureFor(skill, synonymIndex) {
    const parts = [];
    parts.push(skill.name);
    if (skill.description)
        parts.push(skill.description);
    const kw = skill.frontmatter?.keywords;
    if (Array.isArray(kw))
        parts.push(kw.join(' '));
 ...
 */
export function detectSimilarSkills(skills, options = {}) {
    ...
    for (let i = 0; i < skills.length; i++) {
        for (let j = i + 1; j < skills.length; j++) {
            ...
            const b = skills[j];
            if (a.name === b.name)
                continue; // covered by conflict detection
            const pairKey = a.id < b.id ? `${a.id}|${b.id}` : `${b.id}|${a.id}`;
            if (ignored.has(pairKey))
                continue;
