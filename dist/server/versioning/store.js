 ...
 */
async function readDirRecursive(baseDir, currentDir, result) {
    let entries;
    try {
 ...
 */
async function restoreFiles(baseDir, files) {
 ...
 * 创建一个版本快照
 */
export async function createSnapshot(skillPath, skillName, message, source = 'manual') {
    await initStore();
    ...
    await fs.writeFile(path.join(dir, `${id}.json`), JSON.stringify(version, null, 2), 'utf-8');
    return {
        id: version.id,
 ...
 * 获取版本历史（按时间倒序）
 */
export async function getHistory(skillPath) {
    const dir = storeDir(skillPath);
    let entries;
    try {
 ...
 * 获取某个版本的完整内容
 */
export async function getVersion(skillPath, versionId) {
    const dir = storeDir(skillPath);
 ...
 * 对比两个版本的 diff
 */
export async function diffVersions(skillPath, oldId, newId) {
 ...
 * 对比当前文件和某个版本的 diff
 */
export async function diffWithCurrent(skillPath, versionId) {
 ...
 * 回滚到指定版本
 */
export async function rollback(skillPath, versionId) {
    const ver = await getVersion(skillPath, versionId);
    if (!ver)
        return false;
    let realPath;
 ...
 * 删除某个版本
 */
export async function deleteVersion(skillPath, versionId) {
    const dir = storeDir(skillPath);
    try {
...
function longestCommonSubsequence(a, b) {
    ...
    const mm = aa.length;
    const nn = bb.length;
    const dp = Array.from({ length: mm + 1 }, () => Array(nn + 1).fill(0));
    for (let i = 1; i <= mm; i++) {
        for (let j = 1; j <= nn; j++) {
            if (aa[i - 1] === bb[j - 1]) {
                dp[i][j] = dp[i - 1][j - 1] + 1;
