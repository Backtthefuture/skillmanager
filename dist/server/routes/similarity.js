...
async function saveIgnored(list) {
    await fs.mkdir(STORE_DIR, { recursive: true });
    await fs.writeFile(IGNORED_FILE, JSON.stringify(list, null, 2), 'utf-8');
}
export async function similarityRoutes(app) {
    app.get('/api/similar', async (req) => {
        let result = getCachedResult();
        if (!result)
            result = await fullScan();
        ...
        const groups = detectSimilarSkills(result.skills, {
            threshold,
            ignoredPairs: ignored.map((p) => [p.a, p.b]),
        });
        return {
            threshold,
    ...
    app.post('/api/similar/ignore', async (req, reply) => {
        const { a, b } = req.body || {};
        if (!a || !b) {
    ...
    app.post('/api/similar/unignore', async (req, reply) => {
        const { a, b } = req.body || {};
        if (!a || !b) {
            return reply.status(400).send({ error: 'Missing skill ids (a, b)' });
        }
